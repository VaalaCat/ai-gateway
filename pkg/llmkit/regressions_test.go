...
func TestRegression_CodexWebSearchResponsesToChat(t *testing.T) {
	...
	cfg := &testChannelConfig{
		...
		APIKey:  "k",
		Model:   irReq.Model,
		OnDroppedTools: func(tools []codec.DroppedTool) {
			dropped = append([]codec.DroppedTool(nil), tools...)
		},
	...
	// 核心断言 2：公共 OnDroppedTools callback 收到恰好 1 条 web_search。
...
func TestRegression_CodexToolCallChatStreamToResponses(t *testing.T) {
	...
	respInbound := getInbound(codec.ProtocolOpenAIResponses)
	chatOutbound := getOutbound(codec.ProtocolOpenAIChat)
...
func TestRegression_ProtocolOverride_ResponsesToClaude(t *testing.T) {
	...
	outbound := getOutbound(codec.ProtocolClaude)
	if outbound == nil {
...
func TestRegression_ModelProtocolOverride_ExactResponsesToChat(t *testing.T) {
	...
	cfg := &testChannelConfig{BaseURL: "http://stub", APIKey: "k", Model: "gpt-4o"}
	outHTTPReq, err := outbound.EncodeRequest(irReq, cfg)
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
...
func TestRegression_ModelProtocolOverride_RegexWildcardToClaude(t *testing.T) {
	...
	inbound := getInbound(codec.ProtocolOpenAIChat)
	if inbound == nil {
	...
	cfg := &testChannelConfig{BaseURL: "http://stub", APIKey: "k", Model: "deepseek-r1"}
	outHTTPReq, err := outbound.EncodeRequest(irReq, cfg)
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
