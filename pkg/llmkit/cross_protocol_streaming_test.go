...
func decodeFixtureForProto(t *testing.T, proto string, scenario string, outboundCodec testOutboundCodec) []codec.Event {
	t.Helper()
...
func encodeEventsForProto(t *testing.T, inboundCodec testInboundCodec, irEvents []codec.Event) []byte {
	t.Helper()
...
//  1. Decodes an upstream SSE fixture through the public Codec.
//  2. Verifies the IR event stream satisfies AssertStreamingToolCallInvariant.
//  3. Encodes the IR events through the public Codec.
		...
		for _, out := range protos {
			for _, sc := range scenarios {
				...
				t.Run(name, func(t *testing.T) {
					...
					outboundCodec := getOutbound(in.protocol)
					inboundCodec := getInbound(out.protocol)
					...
					if err := assertStreamingToolCallInvariant(irEvents); err != nil {
...
func TestChatToClaude_NamedToolStopProducesCompleteToolUse(t *testing.T) {
	...
	chatCodec := getOutbound(codec.ProtocolOpenAIChat)
	resp := &http.Response{
	...
	claudeCodec := getInbound(codec.ProtocolClaude)
