...
func TestDecodeRequest_WithSystemPrompt(t *testing.T) {
	...
	if req.Messages[0].Role != ir.RoleSystem {
		t.Errorf("first message role = %q, want %q", req.Messages[0].Role, ir.RoleSystem)
	}
	...
	if req.Messages[1].Role != ir.RoleUser {
		t.Errorf("second message role = %q, want %q", req.Messages[1].Role, ir.RoleUser)
	}
...
func TestDecodeRequest_ToolUseAndToolResult(t *testing.T) {
	...
	if req.Messages[2].Role != ir.RoleTool {
		t.Errorf("tool result role = %q, want %q", req.Messages[2].Role, ir.RoleTool)
	}
...
func TestEncodeRequest_ToolsUseInputSchema(t *testing.T) {
	...
	req := &ir.Request{
		Model:     "claude-sonnet-4-20250514",
		MaxTokens: 1024,
		Messages:  []ir.Message{ir.TextMessage(ir.RoleUser, "weather?")},
		Tools: []ir.Tool{
...
func TestDecodeResponse_NonStream(t *testing.T) {
	...
	if events[1].Type != ir.EventContentDelta || events[1].Delta.Text != "Hello!" {
		t.Errorf("event[1] = %+v", events[1])
	}

	// Find usage event
	var usageEvent *ir.Event
	for i := range events {
		if events[i].Type == ir.EventUsage {
			usageEvent = &events[i]
			break
		}
	}
...
func TestInterfaceCompliance(t *testing.T) {
	var _ protocol.Handler = NewHandler()
}
...
func TestClaudeEncodeRequest_UnknownContentBlock_RoundTrip(t *testing.T) {
	...
	cfg := &channelConfig{BaseURL: "https://api.anthropic.com", APIKey: "k", Model: "claude-sonnet-4-20250514"}
	httpReq, err := c.encodeHTTPRequest(req, cfg)
	if err != nil {
...
func TestClaudeEncodeRequest_BetaQuery(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := &handler{}
			req := &ir.Request{Model: "claude", Messages: []ir.Message{ir.TextMessage(ir.RoleUser, "hello")}}
			cfg := &channelConfig{
				BaseURL:         "https://api.anthropic.com",
				APIKey:          "sk-test",
				Model:           "claude",
...
func TestClaudeRequestFieldsRoundTrip(t *testing.T) {
	...
	c := &handler{}
	req, err := c.decodeHTTPRequest(inbound)
	if err != nil {
		t.Fatal(err)
	}
	...
	outbound, err := c.encodeHTTPRequest(req, &channelConfig{BaseURL: "https://api.anthropic.com", APIKey: "k", Model: req.Model})
	if err != nil {
		t.Fatal(err)
	}
