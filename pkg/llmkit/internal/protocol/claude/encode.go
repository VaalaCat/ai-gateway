...
func (c *handler) encodeHTTPRequest(req *ir.Request, cfg *channelConfig) (*http.Request, error) {
	...
	for _, m := range messages {
		...
		if m.Role == ir.RoleTool {
			...
			block := claudeContentBlock{
				Type:      string(ir.ContentTypeToolResult),
			...
			for _, cb := range filtered {
				if cb.RawJSON != nil {
					...
					case ir.ContentTypeText:
						b, _ := json.Marshal(claudeContentBlock{Type: string(ir.ContentTypeText), Text: cb.Text})
						rawBlocks = append(rawBlocks, b)
					case ir.ContentTypeImage:
						block := claudeContentBlock{Type: string(ir.ContentTypeImage)}
						if cb.MediaB64 != "" {
							block.Source = &claudeImageSource{
								Type:      ir.ImageSourceBase64,
								MediaType: cb.MimeType,
								Data:      cb.MediaB64,
							}
						} else if cb.MediaURL != "" {
							block.Source = &claudeImageSource{
								Type: ir.ImageSourceURL,
								URL:  cb.MediaURL,
							}
						}
			...
			for _, tc := range m.ToolCalls {
				...
				b, _ := json.Marshal(claudeContentBlock{
					Type:  string(ir.ContentTypeToolUse),
					ID:    tc.ID,
					Name:  tc.Name,
					Input: input,
				})
			...
			if len(rawBlocks) == 1 && !hasRawJSON && len(m.ToolCalls) == 0 {
				...
				if peek.Type == string(ir.ContentTypeText) {
	...
	if len(req.Tools) > 0 {
		...
		for _, t := range req.Tools {
			r, err := convert.ResolveTool(t, req.InboundProtocol, ir.ProtocolClaude, policy, emit)
			if err != nil {
				return nil, err
			}
			if r.Dropped != nil {
