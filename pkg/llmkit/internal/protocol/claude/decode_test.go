...
func TestDecodeRequest_SystemArray(t *testing.T) {
	...
	if sysMsg.Role != ir.RoleSystem {
		t.Fatalf("first message role = %q, want %q", sysMsg.Role, ir.RoleSystem)
	}

	// The system message should contain text from both array elements
	var sysText string
	for _, cb := range sysMsg.Content {
		if cb.Type == ir.ContentTypeText {
			sysText += cb.Text + " "
		}
	}
...
func TestDecodeRequest_ThinkingContentBlocks(t *testing.T) {
	...
	var assistantMsg *ir.Message
	for i := range req.Messages {
		if req.Messages[i].Role == ir.RoleAssistant {
...
func TestDecodeRequest_MultiToolResult(t *testing.T) {
	...
	for _, m := range req.Messages {
		for _, cb := range m.Content {
			if cb.Type == ir.ContentTypeText && strings.Contains(cb.Text, "compare") {
...
data: {"type":"message_stop"}`
	...
	for ev := range ch {
		if ev.Type == ir.EventRawPassthrough && ev.RawPassthrough != nil && ev.RawPassthrough.EventName == "ping" {
...
func TestDecodeStream_EmitsContentBlockStop(t *testing.T) {
	...
	for _, ev := range events {
		if ev.Type == ir.EventContentBlockStop {
			hasBlockStop = true
			break
		}
	}
...
data: {"type":"message_stop"}`
	...
	var starts, argDeltas, ends []ir.Event
	for _, ev := range events {
		switch ev.Type {
	...
	// Invariant check.
	if err := convert.AssertStreamingToolCallInvariant(events); err != nil {
...
func TestHandlerSetsInboundProtocol(t *testing.T) {
	...
	if req.InboundProtocol != ir.ProtocolClaude {
		t.Errorf("want %q, got %q", ir.ProtocolClaude, req.InboundProtocol)
	}
}
