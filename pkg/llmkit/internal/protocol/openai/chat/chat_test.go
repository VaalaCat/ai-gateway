...
func TestDecodeRequest_MultiContent(t *testing.T) {
	...
	if msg.Content[0].Type != ir.ContentTypeText || msg.Content[0].Text != "What is in this image?" {
		t.Errorf("block[0] = %+v, want text block", msg.Content[0])
	}
	if msg.Content[1].Type != ir.ContentTypeImage || msg.Content[1].MediaURL != "https://example.com/img.png" {
...
func TestDecodeRequest_ToolCalls(t *testing.T) {
	...
	if toolMsg.Role != ir.RoleTool || toolMsg.ToolCallID != "call_abc" {
...
func TestDecodeResponse_NonStream(t *testing.T) {
	...
	for _, ev := range events {
		if ev.Type == ir.EventContentDelta && ev.Delta != nil && ev.Delta.Text == "Hello there!" {
			found = true
			break
		}
	}
...
`
	...
	var startEvents []ir.Event
	var argDeltaEvents []ir.Event
	for ev := range ch {
		switch ev.Type {
...
func TestChatEncodeRequest_WithExtendedIRFields(t *testing.T) {
	...
	c := &handler{}
	cfg := &channelConfig{BaseURL: "https://api.example.com", APIKey: "sk-test", Model: "gpt-5"}
...
func TestEncodeRequest_NoOrg(t *testing.T) {
	req := &ir.Request{
		Model:    "gpt-3.5-turbo",
		Messages: []ir.Message{ir.TextMessage(ir.RoleUser, "test")},
	}
	cfg := &channelConfig{
		BaseURL: "https://api.openai.com",
		APIKey:  "sk-key",
...
func TestEncodeResponse_StreamToolCalls(t *testing.T) {
	ch := make(chan ir.Event, 10)
	ch <- ir.Event{Type: ir.EventStreamStart}
	ch <- ir.Event{Type: ir.EventToolCallStart, ToolCall: &ir.StreamingToolCall{CallID: "call_2", Index: 0, Name: "search"}}
	ch <- ir.Event{Type: ir.EventToolCallArgumentsDelta, ToolCall: &ir.StreamingToolCall{CallID: "call_2", Arguments: `{"q":"test"}`}}
	ch <- ir.Event{Type: ir.EventToolCallEnd, ToolCall: &ir.StreamingToolCall{CallID: "call_2"}}
	ch <- ir.Event{Type: ir.EventDone}
	close(ch)
...
func TestEncodeResponse_FinishReasonLength(t *testing.T) {
	...
	ch <- ir.Event{FinishReason: "length"}
	ch <- ir.Event{
		Type: ir.EventUsage,
		Usage: &ir.Usage{
			PromptTokens:     10,
...
func TestEncodeRequest_WithTools(t *testing.T) {
	req := &ir.Request{
		Model:    "gpt-4",
		Messages: []ir.Message{ir.TextMessage(ir.RoleUser, "search for cats")},
		Tools: []ir.Tool{
			{
				Name:        "search",
...
func TestEncodeRequest_WithToolCalls(t *testing.T) {
	req := &ir.Request{
		Model: "gpt-4",
		Messages: []ir.Message{
			ir.TextMessage(ir.RoleUser, "What time is it?"),
			{
			...
			},
			{
				...
				Content: []ir.ContentBlock{
					{Type: ir.ContentTypeText, Text: "14:30 UTC"},
				},
			},
		},
	}
...
`
	...
	for _, ev := range events {
		if ev.Type == ir.EventThinkingDelta && ev.Delta != nil {
			if ev.Delta.ContentType != ir.ContentTypeThinking {
				t.Errorf("thinking delta ContentType = %q, want %q", ev.Delta.ContentType, ir.ContentTypeThinking)
			}
		}
	}
}
...
// survives a full decode → encode round trip through the ir.
...
`
	...
	c := &handler{}
	decodedCh, err := c.decodeHTTPResponse(resp, true)
	if err != nil {
		t.Fatalf("DecodeResponse: %v", err)
	}

	// Collect events and re-encode
	var allEvents []ir.Event
	for ev := range decodedCh {
		allEvents = append(allEvents, ev)
	}

	// Feed into encode
	encodeCh := make(chan ir.Event, len(allEvents))
	...
	rec := httptest.NewRecorder()
	w := &flushRecorder{ResponseRecorder: rec}
	err = c.encodeHTTPResponse(encodeCh, w, true)
	if err != nil {
		t.Fatalf("EncodeResponse: %v", err)
	}
