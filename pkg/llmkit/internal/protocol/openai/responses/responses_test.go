...
func TestResponsesEncodeRequest(t *testing.T) {
	...
	req := &ir.Request{
		Model: "gpt-4o",
		Messages: []ir.Message{
			ir.TextMessage(ir.RoleSystem, "Be helpful"),
			ir.TextMessage(ir.RoleUser, "Hi there"),
		},
		Stream:      true,
		MaxTokens:   512,
		Temperature: &temp,
...
func TestResponsesDecodeResponse_NonStream(t *testing.T) {
	...
	c := &handler{}
	ch, err := c.decodeHTTPResponse(resp, false)
	if err != nil {
	...
	for _, ev := range events {
		if ev.Type == ir.EventContentDelta && ev.Delta != nil && ev.Delta.Text == "Hello!" {
			found = true
			break
		}
	}
	...
	foundUsage := false
	for _, ev := range events {
		if ev.Type == ir.EventUsage && ev.Usage != nil {
	...
	if events[len(events)-1].Type != ir.EventDone {
...
`
	...
	c := &handler{}
	ch, err := c.decodeHTTPResponse(resp, true)
	if err != nil {
		t.Fatalf("DecodeResponse: %v", err)
	}
...
`
	...
	thinkingCount := 0
	for _, ev := range events {
		if ev.Type == ir.EventThinkingDelta && ev.Delta != nil {
...
func TestResponsesDecodeRequest_FunctionCallOutput(t *testing.T) {
	...
	if len(req.Messages[2].Content) != 1 || req.Messages[2].Content[0].Type != ir.ContentTypeText {
...
func TestResponsesDecodeRequest_ContentArray(t *testing.T) {
	...
	if req.Messages[0].Content[0].Type != ir.ContentTypeText {
		t.Errorf("msg[0].content[0].type = %v, want text", req.Messages[0].Content[0].Type)
	}
	if req.Messages[0].Content[1].Type != ir.ContentTypeText {
...
func TestResponsesDecodeResponse_NonStreamToolCalls(t *testing.T) {
	...
	foundTC := false
	for _, ev := range events {
		if ev.Type == ir.EventToolCallDelta && ev.Delta != nil && ev.Delta.ToolCall != nil {
...
func TestResponsesEncodeRequest_WithToolMessages(t *testing.T) {
	req := &ir.Request{
		...
		Messages: []ir.Message{
			ir.TextMessage(ir.RoleSystem, "Be helpful"),
			ir.TextMessage(ir.RoleUser, "Weather?"),
			{
				Role:       ir.RoleTool,
				ToolCallID: "call_1",
				Content: []ir.ContentBlock{
					{Type: ir.ContentTypeText, Text: "22C"},
				},
			},
		},
		Stream: false,
...
func TestResponsesEncodeResponse_StreamToolCalls(t *testing.T) {
	...
	ch <- ir.Event{
		...
		ToolCall: &ir.StreamingToolCall{CallID: "call_1", Index: 0, Name: "get_weather"},
	}
	ch <- ir.Event{
		Type:     ir.EventToolCallArgumentsDelta,
		ToolCall: &ir.StreamingToolCall{CallID: "call_1", Arguments: `{"city":"NYC"}`},
	}
	ch <- ir.Event{
...
func TestResponsesDecodeRequest_MultiRoleMixed(t *testing.T) {
	...
	if req.Messages[3].Role != ir.RoleUser {
...
func TestResponsesDecodeResponse_NonStreamMixedTextAndToolCalls(t *testing.T) {
	...
	foundTC := false
	for _, ev := range events {
		if ev.Type == ir.EventContentDelta && ev.Delta != nil && ev.Delta.Text == "Let me check the weather." {
			foundText = true
		}
		if ev.Type == ir.EventToolCallDelta && ev.Delta != nil && ev.Delta.ToolCall != nil && ev.Delta.ToolCall.Name == "get_weather" {
			foundTC = true
		}
	}
	if !foundText {
...
`
	...
	foundError := false
	for _, ev := range events {
		if ev.Type == ir.EventError && ev.Error != nil {
	...
	foundPartial := false
	for _, ev := range events {
		if ev.Type == ir.EventContentDelta && ev.Delta != nil && ev.Delta.Text == "partial" {
			foundPartial = true
		}
	}
	if !foundPartial {
...
`
	...
	if lastEvent.Type != ir.EventDone {
...
`
	...
	foundUsage := false
	for _, ev := range events {
		if ev.Type == ir.EventThinkingDelta && ev.Delta != nil && ev.Delta.Text == "Thinking..." {
			foundThinking = true
		}
		if ev.Type == ir.EventContentDelta && ev.Delta != nil && ev.Delta.Text == "Let me check." {
			foundContent = true
		}
		if ev.Type == ir.EventToolCallStart && ev.ToolCall != nil && ev.ToolCall.Name == "search" {
			foundTC = true
		}
...
`
	...
	foundContent := false
	for _, ev := range events {
		if ev.Type == ir.EventContentDelta && ev.Delta != nil && ev.Delta.Text == "result" {
			foundContent = true
		}
	}
	if !foundContent {
...
`
	...
	foundContent := false
	for _, ev := range events {
		if ev.Type == ir.EventContentDelta && ev.Delta != nil && ev.Delta.Text == "ok" {
			foundContent = true
		}
	}
	if !foundContent {
...
func TestResponsesEncodeResponse_NonStreamWithThinking(t *testing.T) {
	...
	ch <- ir.Event{
		Type:  ir.EventThinkingDelta,
		Delta: &ir.DeltaPayload{ContentType: ir.ContentTypeThinking, Text: "Let me think."},
	}
	ch <- ir.Event{
		Type:  ir.EventContentDelta,
		Delta: &ir.DeltaPayload{ContentType: ir.ContentTypeText, Text: "Answer"},
	}
	ch <- ir.Event{Type: ir.EventDone}
	close(ch)
	...
	w := httptest.NewRecorder()
	c := &handler{}
	if err := c.encodeHTTPResponse(ch, w, false); err != nil {
		t.Fatalf("EncodeResponse: %v", err)
	}
...
func TestResponsesEncodeResponse_NonStreamMixedTextAndToolCalls(t *testing.T) {
	...
	ch <- ir.Event{
		Type:  ir.EventContentDelta,
		Delta: &ir.DeltaPayload{ContentType: ir.ContentTypeText, Text: "Checking..."},
	}
	ch <- ir.Event{
		Type: ir.EventToolCallDelta,
		Delta: &ir.DeltaPayload{
			ToolCall: &ir.ToolCallDelta{ID: "c1", Name: "search", Arguments: `{"q":"test"}`},
		},
	}
	ch <- ir.Event{Type: ir.EventDone}
	close(ch)
...
func TestResponsesEncodeResponse_StreamError(t *testing.T) {
	...
	ch <- ir.Event{
		Type:  ir.EventError,
		Error: &ir.ErrorPayload{Code: "rate_limit_exceeded", Message: "Too many requests"},
	}
	close(ch)
...
`
	...
	ptCount := 0
	for _, ev := range events {
		if ev.Type == ir.EventRawPassthrough && ev.RawPassthrough != nil {
...
func TestResponsesEncodeStream_FunctionCallOnlyResponse(t *testing.T) {
	...
	ch <- ir.Event{
		Type:     ir.EventToolCallStart,
		ToolCall: &ir.StreamingToolCall{CallID: "call_1", Index: 0, Name: "exec_command"},
	}
	ch <- ir.Event{
		Type:     ir.EventToolCallArgumentsDelta,
		ToolCall: &ir.StreamingToolCall{CallID: "call_1", Arguments: `{"cmd":"ls"}`},
	}
	ch <- ir.Event{
		Type:     ir.EventToolCallEnd,
		ToolCall: &ir.StreamingToolCall{CallID: "call_1", Arguments: `{"cmd":"ls"}`},
...
func TestResponsesEncodeStream_TextResponseWithUsage(t *testing.T) {
	...
	ch <- ir.Event{
		Type:  ir.EventContentDelta,
		Delta: &ir.DeltaPayload{ContentType: ir.ContentTypeText, Text: "Hello"},
	}
	ch <- ir.Event{
		Type: ir.EventUsage,
		Usage: &ir.Usage{
...
func TestHandler_RoundTrip_ExtendedFields(t *testing.T) {
	ptc := true
	store := false
	strict := false
	...
	irReq := &ir.Request{
		...
		Messages: []ir.Message{
			ir.TextMessage(ir.RoleSystem, "Be helpful"),
			ir.TextMessage(ir.RoleUser, "hello"),
		},
		Stream:            true,
	...
	c := &handler{}
	httpReq, err := c.encodeHTTPRequest(irReq, cfg)
	if err != nil {
		t.Fatalf("EncodeRequest: %v", err)
	}
...
func TestHandler_RealWorldCodexRequest_RoundTrip(t *testing.T) {
	...
	c := &handler{}
	irReq, err := c.decodeHTTPRequest(r)
	if err != nil {
		t.Fatalf("DecodeRequest: %v", err)
	}
	...
	if irReq.Messages[0].Role != ir.RoleSystem {
...
func TestHandler_RoundTrip_UnknownFields(t *testing.T) {
	body := `{
		"model": "gpt-4o",
		"input": "What is AI?",
		"stream": false,
...
func TestResponsesDecodeStream_UnknownEventsPassthrough(t *testing.T) {
	...
	var passthroughs []ir.Event
	var knownEvents []ir.Event
	for _, ev := range events {
		if ev.Type == ir.EventRawPassthrough {
	...
	if knownEvents[1].Type != ir.EventContentDelta {
...
func TestResponsesStream_DecodeEncode_Roundtrip_WithPassthrough(t *testing.T) {
	...
	c := &handler{}
	decodeCh, err := c.decodeHTTPResponse(resp, true)
	if err != nil {
		t.Fatalf("DecodeResponse: %v", err)
	}

	var irEvents []ir.Event
	...
	encodeCh := make(chan ir.Event, len(irEvents))
	for _, ev := range irEvents {
		encodeCh <- ev
	}
	close(encodeCh)
	...
	w := httptest.NewRecorder()
	if err := c.encodeHTTPResponse(encodeCh, w, true); err != nil {
		t.Fatalf("EncodeResponse: %v", err)
	}
...
func TestResponsesStream_RealWorldCodexResponse_Roundtrip(t *testing.T) {
	...
	var (
		...
		startEvents   []ir.Event
		argDeltas     []ir.Event
		endEvents     []ir.Event
		usageEvents   []ir.Event
		doneEvents    int
		passthroughs  []ir.Event
	)
	for _, ev := range irEvents {
		switch ev.Type {
...
func TestResponsesNonStream_Roundtrip_ExtrasPreserved(t *testing.T) {
	...
	c := &handler{}
	decodeCh, err := c.decodeHTTPResponse(resp, false)
	if err != nil {
		t.Fatalf("DecodeResponse: %v", err)
	}
	...
	encodeCh := make(chan ir.Event, len(events))
	for _, ev := range events {
		encodeCh <- ev
	}
	close(encodeCh)
	...
	w := httptest.NewRecorder()
	err = c.encodeHTTPResponse(encodeCh, w, false)
	if err != nil {
		t.Fatalf("EncodeResponse: %v", err)
	}
...
func TestResponsesDecodeStream_LargeResponseCompleted(t *testing.T) {
	...
	hasUsage := false
	for _, ev := range events {
		if ev.Type == ir.EventDone {
			hasDone = true
		}
		if ev.Type == ir.EventUsage && ev.Usage != nil && ev.Usage.CompletionTokens == 500 {
			hasUsage = true
		}
	}
...
func TestResponsesDecodeRequest_UnknownContentBlockType_PreservedAsRawJSON(t *testing.T) {
	...
	if m.Content[0].Type != ir.ContentTypeText {
...
func TestResponsesEncodeRequest_UnknownInputItem_RoundTrip(t *testing.T) {
	...
	cfg := &channelConfig{BaseURL: "https://api.example.com", APIKey: "k", Model: "gpt-4o"}
	httpReq, err := c.encodeHTTPRequest(req, cfg)
	if err != nil {
		t.Fatalf("EncodeRequest: %v", err)
	}
...
func TestHandler_MultiTurnWithUnknownTypes_RoundTrip(t *testing.T) {
	body := `{
		"model": "qwen-flash",
	...
	cfg := &channelConfig{BaseURL: "https://api.example.com", APIKey: "k", Model: "qwen-flash"}
	httpReq, err := c.encodeHTTPRequest(req, cfg)
	if err != nil {
		t.Fatalf("EncodeRequest: %v", err)
	}
...
// roundtripSSE decodes then encodes SSE input through the handler.
...
func TestResponsesStream_CrossProtocol_GeneratesStructuralEvents(t *testing.T) {
	...
	irEvents := []ir.Event{
		...
		{
			Type:  ir.EventContentDelta,
			Delta: &ir.DeltaPayload{ContentType: ir.ContentTypeText, Text: " world"},
		},
		{
			Type:     ir.EventToolCallStart,
			ToolCall: &ir.StreamingToolCall{CallID: "call_1", Index: 0, Name: "run"},
		},
		{
			Type:     ir.EventToolCallArgumentsDelta,
			ToolCall: &ir.StreamingToolCall{CallID: "call_1", Arguments: `{"x":1}`},
		},
		{
			Type:     ir.EventToolCallEnd,
			ToolCall: &ir.StreamingToolCall{CallID: "call_1", Arguments: `{"x":1}`},
