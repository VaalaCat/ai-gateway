...
func collectResponsesStreamEvents(t *testing.T, sseData string) []ir.Event {
	t.Helper()
	resp := &http.Response{
		StatusCode: 200,
...
func TestHandlerSetsInboundProtocol(t *testing.T) {
	...
	if req.InboundProtocol != ir.ProtocolOpenAIResponses {
		t.Errorf("want %q, got %q", ir.ProtocolOpenAIResponses, req.InboundProtocol)
	}
}
