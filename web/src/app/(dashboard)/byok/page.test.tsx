...
const rows: BYOKChannelDetail[] = [
  baseChannel,
  { ...baseChannel, id: 2, name: "Manual BYOK", status: 0 },
  {
    ...baseChannel,
    id: 3,
    name: "Error BYOK",
...
vi.mock("@/lib/api/byok-channels", async (importOriginal) => {
  ...
  return {
    ...actual,
    useBYOKChannels: () => ({ data: { data: rows, total: rows.length }, isLoading: false, refetch: vi.fn() }),
    useBYOKSupportedTypes: () => ({ data: { types: [] } }),
    useDeleteBYOKChannel: () => ({ mutateAsync: vi.fn() }),
    useTestBYOKChannel: () => ({ mutateAsync: vi.fn() }),
    useUpdateBYOKChannel: () => ({ mutate: vi.fn(), isPending: false }),
  };
});
vi.mock("@/components/data-table/selection-column", () => ({
  createSelectionColumn: () => ({ id: "select" }),
}));
vi.mock("@/components/data-table/data-table", () => ({
  DataTable: ({ columns, data }: {
    ...
    return data.map((channel) => (
      <div key={channel.id} data-testid={`byok-${channel.id}`}>
...
vi.mock("@/components/business/channel-transfer-dialogs", () => ({
  ChannelImportDialog: () => null,
  ChannelExportDialog: () => null,
}));
