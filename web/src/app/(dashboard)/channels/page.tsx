    ...
    return (
      <div className="space-y-4">
        ...
        {!channel.use_legacy_adaptor && (
          <div>
            <h4 className="text-sm font-medium mb-2">{t("expandedEndpoints")}</h4>
        ...
        {mappings.length > 0 && (
          <div>
            <h4 className="text-sm font-medium mb-2">{t("modelMapping")}</h4>
            <div className="space-y-1">
        ...
        {/* Configuration Grid */}
        <div>
          <h4 className="text-sm font-medium mb-2">{t("expandedConfig")}</h4>
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1.5">
          ...
          {row.original.use_legacy_adaptor ? (
            <Badge variant="outline" className="text-[10px] px-1 py-0 border-yellow-500/50 text-yellow-600 dark:text-yellow-400">
              {t("modeLegacy")}
            </Badge>
          ) : (
            <Badge variant="outline" className="text-[10px] px-1 py-0 border-blue-500/50 text-blue-600 dark:text-blue-400">
              {t("modeNative")}
            </Badge>
          )}
        </div>
          ...
          <TooltipProvider delayDuration={200}>
            <Tooltip>
              <TooltipTrigger asChild>
                <span className="max-w-[160px] truncate block text-xs font-mono cursor-default">
                  {hostname}
                </span>
              </TooltipTrigger>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (ch.use_legacy_adaptor) {
          return <span className="text-xs text-muted-foreground">{getTypeName(ch.type)}</span>;
        }
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex items-center gap-1 max-w-[280px] overflow-hidden">
                  {show.map((m: string) => (
                    <Badge key={m} variant="secondary" className="text-[11px] font-mono px-1.5 py-0 shrink-0 truncate max-w-[100px]">
                      {m}
                    </Badge>
                  ))}
                  {rest > 0 && (
                    <span className="text-xs text-muted-foreground shrink-0">+{rest}</span>
                  )}
                </div>
              </TooltipTrigger>
              {models.length > 3 && (
    ...
    <div className="space-y-4">
      ...
      <DataTable
        ...
        toolbar={
          <DataTableToolbar
            searchValue={search}
            searchPlaceholder={t("searchNameOrModel")}
            onSearchChange={setSearch}
          >
            <Button onClick={() => router.push("/channels/new")}>
