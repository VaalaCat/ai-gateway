...
vi.mock("@/components/business/agent-connections-panel", () => ({
  AgentConnectionsPanel: ({ snapshot, stale, initialRouteTargetsPage, routeTargetsCurrent, refreshing }: { snapshot: AgentDetail["connection"]; stale: boolean; initialRouteTargetsPage?: AgentDetail["route_targets"]; routeTargetsCurrent: boolean; refreshing: boolean }) => <div>Authoritative connection panel: {snapshot.control.state}; stale={String(stale)}; direct={initialRouteTargetsPage?.snapshot_seq ?? "none"}; targets-current={String(routeTargetsCurrent)}; refreshing={String(refreshing)}</div>,
}));
...
const detail: AgentDetail = {
  id: 7, agent_id: "agent-a", name: "Agent A", status: 1, tags: "edge", proxy_url: "", relay_mode: "inherit", relay_uri: "", direct_inbound_enabled: true, direct_outbound_enabled: true, relay_inbound_enabled: true, relay_outbound_enabled: true, last_seen: 1, created_at: 1, http_addresses: "[]",
...
const connectionQueryState = vi.hoisted(() => ({
  data: undefined as AgentDetail["connection"] | undefined,
  routeTargetsPage: undefined as AgentDetail["route_targets"] | undefined,
...
describe("AgentDetailPage", () => {
  beforeEach(() => {
    ...
    connectionQueryState.routeTargetsPage = undefined;
    connectionQueryState.routeTargetsCurrent = true;
  ...
  it("passes a retained route target page as refreshing and non-current while detail catches up", async () => {
    const user = userEvent.setup();
    connectionQueryState.data = {
      ...detail.connection,
      snapshot_seq: 2,
    ...
    connectionQueryState.routeTargetsPage = detail.route_targets;
    connectionQueryState.routeTargetsCurrent = false;
    connectionQueryState.isFetching = true;
    render(<AgentDetailPage />);

    await user.click(screen.getByRole("tab", { name: "Connections" }));
    expect(screen.getByText(/direct=7; targets-current=false; refreshing=true/)).toBeInTheDocument();
  });
