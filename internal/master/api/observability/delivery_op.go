...
func deliveryOpTargetsForAgent(req DeliveryOpRequest) (queueIDs, requestIDs []string, err error) {
	if req.QueueIDs != nil {
		return req.QueueIDs, []string{}, nil
	}
	for _, requestID := range req.RequestIDs {
		if strings.HasPrefix(requestID, "llm:") || strings.HasPrefix(requestID, "api:") {
			return nil, nil, api.BadRequestError(
				"typed request_ids came from an outdated delivery board; refresh and retry",
				nil,
			)
		}
	}
	return nil, req.RequestIDs, nil
}
...
func (h *Handler) PostDeliveryOp(c *app.Context, req DeliveryOpRequest) (DeliveryOpResponse, error) {
	...
	if req.Op == "drop" && len(req.QueueIDs) == 0 && len(req.RequestIDs) == 0 {
		// 危险操作必须逐条点名,禁止"drop 全部"
		return DeliveryOpResponse{}, api.BadRequestError("drop requires explicit queue_ids or request_ids", nil)
	}
	if req.Op == "degrade" && req.Level != 2 && req.Level != 3 {
		return DeliveryOpResponse{}, api.BadRequestError("degrade level must be 2 or 3", nil)
	}
	queueIDs, requestIDs, err := deliveryOpTargetsForAgent(req)
	if err != nil {
		return DeliveryOpResponse{}, err
	}
	if h.HubCall == nil {
	...
	raw, err := h.HubCall(agent.AgentID, consts.RPCAgentUsageQueueOp, map[string]any{
		"op": req.Op, "queue_ids": queueIDs, "request_ids": requestIDs, "level": req.Level,
	}, 10*time.Second)
	if err != nil {
	...
	if req.Op == "drop" {
		...
		c.Logger.Warn("delivery-op drop executed",
			...
			zap.Strings("queue_ids", queueIDs), zap.Strings("request_ids", requestIDs),
			zap.Int("affected", result.Affected))
	}
	return result, nil
}
