...
func NewRefresher[K comparable, V any](load Loader[K, V], cfg func() RefreshConfig) *Refresher[K, V] {
	return NewRefresherWithLifecycle(load, cfg, NewLifecycle())
}
...
func NewRefresherWithLifecycle[K comparable, V any](load Loader[K, V], cfg func() RefreshConfig, lifecycle *Lifecycle) *Refresher[K, V] {
	return &Refresher[K, V]{load: load, cfg: cfg, lifecycle: lifecycle}
}
...
func (r *Refresher[K, V]) TriggerRefresh(key K, handler func(RefreshOutcome, V)) {
	...
	if !r.lifecycle.GoRefresh(func(ctx context.Context) {
...
func (r *Refresher[K, V]) runOnce(parent context.Context, key K) (RefreshOutcome, V) {
	cfg := r.cfg()
	...
	v, err := failsafe.With[V](retry).WithContext(parent).Get(func() (V, error) {
		ctx, cancel := context.WithTimeout(parent, cfg.RefreshTimeout)
		defer cancel()
